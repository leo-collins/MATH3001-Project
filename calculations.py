...
for h in np.linspace(1.7, 1.7, num=1):
    values = []
    b_h = []
    vac_eigs = DiscretePhiFourSystem(N, h, 0, vac=True).spectra()
    kink_0 = DiscretePhiFourSystem(N, h, 0)
    for b in np.linspace(kink_0.values[N - 1], kink_0.values[N + 1], num=500):
        # values.append(E(b, vac_eigs) - E(0, vac_eigs))
        values.append(E(b, vac_eigs))
        b_h.append(b / kink_0.values[N + 1])
